...
There are 7,041 political candidates in this data set. 
...
In the write-up, describe which type of candidate sponsored the most promoting ads on average. 
...
Democratic Incumbent candidates sponsored the most promoting ads on average, with their promote proportion being 0.854. 
...
## Answer 3
The new variable 'impressions_millions':
```{r}
fb_ads <- fb_ads |>
  mutate(impressions_millions = impressions / 1000000)
```

The histogram of 'impressions_millions' for US House races: 
```{r}
plot_q3 <- ggplot(data = fb_ads, filter(office == "US House"), 
                  mapping = aes(x = impressions_millions)) +
  geom_histogram(binwidth = 1, alpha = 0.7) + 
  labs(title = "Facebook Ad Impressions for US House Candidates", 
       x = "Facebook Ad Impressions (in millions)", 
       y = "Number of US House Candidates") +
  xlim(0,100) +
  ylim(0, 150)
...
This histogram is skewed to the left of 10 million, peaking at around 3 million ad impressions and decreasing exponentially from there. Therefore, most House candidates did not have more than 10 million ad impressions on Facebook. 
